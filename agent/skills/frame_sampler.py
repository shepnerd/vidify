...
PTS_RE = re.compile(r".*_(\d+\.\d+)\.(jpg|png)$")
...
def sample_frames(video, out_dir: str, strategy: FrameStrategy) -> FrameSet:
    ...
    if strategy.type == "fps":
        ...
        # 文件名包含 pts_time
        out_tpl = os.path.join(out_dir, "f_%06d_%0.3f.jpg")
        cmd = ["ffmpeg", "-y", "-i", video.local_path,
               "-vf", f"fps={fps},scale=512:256:force_original_aspect_ratio=decrease",
               "-q:v", "2", out_tpl]
        _run(cmd)
        ...
        vf = f"select='gt(scene,{th})',scale=512:256:force_original_aspect_ratio=decrease"
        cmd = ["ffmpeg", "-y", "-i", video.local_path,
